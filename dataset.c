...
char * hospital[] = {"KEM Hospital Mumbai", "Sahyadri Hospital Pune", "Vardaan Hospital Delhi"};
...
//	char username[21] = "aaaaaa";
	...
	for(i = start; i < start + n; i++)
	{
		...
		fprintf(stdout, "\t\"approved\" : "); fprintf(stdout, "\"%d\",\n", hack = approved[getRandom(sizeof(approved)/sizeof(approved[0]))]);
...
//		fprintf(stdout, "\t},\n");
		fprintf(stdout, "\t\"hospital\" : "); fprintf(stdout, "\"%s\",\n", hospital[hack = getRandom(sizeof(hospital)/sizeof(hospital[0]))]);
		fprintf(stdout, "\t\"Doc\" : "); fprintf(stdout, "\"%s\"\n", doctor[hack]);
